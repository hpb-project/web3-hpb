...
public class Sign {
    ...
    static byte[] getHpbMessagePrefix(int messageLength) {
        return MESSAGE_PREFIX.concat(String.valueOf(messageLength)).getBytes();
    }
    static byte[] getHpbMessageHash(byte[] message) {
        byte[] prefix = getHpbMessagePrefix(message.length);
    ...
    public static SignatureData signPrefixedMessage(byte[] message, ECKeyPair keyPair) {
        return signMessage(getHpbMessageHash(message), keyPair, false);
    }
    ...
    public static String sign(String message,String privateKey, boolean needToHash) {
		ECKeyPair ecKeyPair =ECKeyPair.create(Numeric.toBigInt(privateKey));
		SignatureData signatureData = signMessage(message.getBytes(StandardCharsets.UTF_8), ecKeyPair,needToHash);
    ...
    public static boolean verifyTransaction(String hexTransaction,String address,long chainId) throws SignatureException{
		...
		byte b = rawTransaction.getRealV(v);
		int header = b & 0xFF;
		...
		if(Numeric.toBigInt(address).equals(Numeric.toBigInt(Keys.getAddress(publicKey)))) {
			return verify(msg, signature, key);
		}
    	return false;
    }
    public static boolean verify(String signMessage,String address, boolean needToHash) throws SignatureException{
	...
	private static byte[] recoverPubBytesFromSignature(int recId, ECDSASignature sig, byte[] message) {
    ...
    public static BigInteger signedPrefixedMessageToKey(byte[] message, SignatureData signatureData)
            throws SignatureException {
        return signedMessageHashToKey(getHpbMessageHash(message), signatureData);
    }
