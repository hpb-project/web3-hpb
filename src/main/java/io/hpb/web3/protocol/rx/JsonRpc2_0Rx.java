...
public class JsonRpc2_0Rx {
    ...
    public Flowable<String> hpbBlockHashFlowable(long pollingInterval) {
        return Flowable.create(
                subscriber -> {
                    BlockFilter blockFilter = new BlockFilter(web3, subscriber::onNext);
        ...
        return Flowable.create(
                subscriber -> {
                    PendingTransactionFilter pendingTransactionFilter =
                            new PendingTransactionFilter(web3, subscriber::onNext);
    ...
    public Flowable<Log> hpbLogFlowable(
        ...
        return Flowable.create(
                subscriber -> {
                    LogFilter logFilter = new LogFilter(web3, subscriber::onNext, hpbFilter);
                    run(logFilter, subscriber, pollingInterval);
                },
                BackpressureStrategy.BUFFER);
    }
    private <T> void run(
            io.hpb.web3.protocol.core.filters.Filter<T> filter,
    ...
    public Flowable<HpbBlock> blockFlowable(boolean fullTransactionObjects, long pollingInterval) {
        return hpbBlockHashFlowable(pollingInterval)
                .flatMap(
                        blockHash ->
                                web3.hpbGetBlockByHash(blockHash, fullTransactionObjects)
                                        .flowable());
    }
    ...
    private BigInteger getBlockNumber(DefaultBlockParameter defaultBlockParameter)
        ...
        if (defaultBlockParameter instanceof DefaultBlockParameterNumber) {
            ...
            HpbBlock latestHpbBlock =
                    web3.hpbGetBlockByNumber(defaultBlockParameter, false).send();
